...
if (isset($_SESSION["user_id"]) && isset($_SESSION["username"])) {
  ...
  try {
    $result = mysqli_query($conn, "SELECT count(*) FROM customers WHERE deleted='0' AND customer_id!='1';");
  } catch (Exception $e) {
  ...
  if (($page * $records) > ($record_count + $records)) {
    header("Location: manage_customers?page=" . $page_count . "&records=" . $records);
  } else {
    try {
      $query = "SELECT * FROM customers WHERE deleted='0' AND customer_id!='1' ORDER BY customer_id LIMIT $records OFFSET $start_from;";
  ...
  <body>
    ...
    <section class="content">
      ...
      <div>
        <div class="button-div">
          <a class="btn btn-primary" href="add_customer">Add New</a>
          <button class="btn btn-danger btn-delete hidden" onclick="deleteCustomers()">Delete Selected</button>
        </div>
        <div class="search-div">
          <div class="input-group">
            <input type="text" class="form-control" name="customerId" id="customerId" value="" placeholder="Search Customers" />
            <span class="input-group-text btn-search" name="search" title="Search Customer" onclick="searchCustomer()">
              <i class="bi bi-search"></i>
            </span>
          </div>
        </div>
        <a class="btn btn-primary btn-dashboard hidden"
          href="manage_customers?page=<?php echo $page . "&records=" . $records; ?>">Back to Customers</a>
    ...
    <script>
      ...
      function deleteCustomers() {
        if (confirm("Are you sure you want to delete these customers ?")) {
          $.post("delete_customers",
            { "customerIds": JSON.stringify(checkList) },
            function (data) {
              if (data === "success") {
                window.location.replace("manage_customers?page=" + "<?php echo $page ?>" + "&records=" + "<?php echo $records ?>");
              } else {
                $(".msg-box").html(data);
              }
            }
      ...
      function applyPagination(element) {
        ...
        window.location.replace("manage_customers?page=" + pageElements[3].value + "&records=" + pageElements[8].value);
      }

      function firstPage() {
        const recordSize = "<?php echo $records ?>";
        const page = 1;
        window.location.replace("manage_customers?page=" + page + "&records=" + recordSize);
      }
