    ...
    switch ($_POST["operation"]) {
      ...
      case 'setCustomerType': {
        ...
        if ($customer_type !== "0") {
          if ($customer_type === "walkin") {
            $_SESSION["edit_customer_id"] = "1";
            $_SESSION["edit_customer_name"] = "Walk-In";
          } else if ($customer_type === "new") {
            unset($_SESSION["edit_customer_id"]);
            unset($_SESSION["edit_customer_name"]);
          } else {
            $_SESSION["edit_customer_type"] = $customer_type;
          }
          echo "success";
        } else {
          ...
          try {
            ...
            if (mysqli_num_rows($validationResult) > 0) {
              ...
              if (mysqli_affected_rows($conn) > 0) {
                $_SESSION["edit_customer_type"] = "existing";
                $_SESSION["edit_customer_id"] = strval(mysqli_insert_id($conn));
                $_SESSION["edit_customer_name"] = $customer_name;
                echo "success";
              }
            }
          } catch (Exception $e) {
      ...
      case 'setCustomer': {
        ...
        if ($customer_id === "0") {
          ...
          $_SESSION["edit_customer_id"] = $customer_id;
          $_SESSION["edit_customer_name"] = $customer_name;
          echo "success";
        }
        break;
      ...
      case 'setSaleDate': {
        $_SESSION["edit_sale_date"] = $_POST["saleDate"];
        break;
      }
      ...
      case 'addProductToSale': {
        ...
        try {
          ...
          if (mysqli_num_rows($result) > 0) {
            ...
            $entryPresent = false;
            for ($i = 0; $i < count($editSaleProductList); $i++) {
              if ($editSaleProductList[$i]["product_id"] === $id) {
                $price = floatval($editSaleProductList[$i]["selling_price"]);
                $qty = intval($editSaleProductList[$i]["sale_qty"]) + 1;
                $skuList = $editSaleProductList[$i]["sku_list"];
                if (!in_array($sku, $skuList)) {
                  array_push($skuList, $sku);
                  $_SESSION["editSaleProductList"][$i]["sale_qty"] = strval($qty);
                  $_SESSION["editSaleProductList"][$i]["sub_total"] = number_format($price * $qty, 2, '.', '');
                  $_SESSION["editSaleProductList"][$i]["sku_list"] = $skuList;
                  if (isset($editSaleProductList[$i]["edited"])) {
                    $_SESSION["editSaleProductList"][$i]["edited"] = "Y";
                  }
                }
                $entryPresent = true;
                break;
      ...
      case 'removeProductFromSale': {
        ...
        $edit_sale_invoice_no = $_SESSION["edit_sale_invoice_no"];
        $query = "SELECT * FROM sku WHERE product_id='$id' AND sale_invoice_no='$edit_sale_invoice_no' AND status!='sold';";
        $editSaleProductList = $_SESSION["editSaleProductList"] ?? [];
        try {
          $entryPresent = false;
          $result = mysqli_query($conn, $query);
          if (mysqli_num_rows($result) > 0) {
            $_SESSION["editSaleError"] = "This product cannot be deleted from the sale as it has been returned.";
            echo "error";
          } else {
            for ($i = 0; $i < count($editSaleProductList); $i++) {
              if ($editSaleProductList[$i]["product_id"] === $id) {
            ...
            if ($entryPresent) {
              if (isset($editSaleProductList[$i]["deleted"])) {
                $_SESSION["editSaleProductList"][$i]["deleted"] = "Y";
              } else {
                array_splice($editSaleProductList, $i, 1);
                $_SESSION["editSaleProductList"] = $editSaleProductList;
              }
            }
            echo "success";
          ...
          if (preg_match('/^\d{0,8}(\.\d{1,2})?$/', $salePrice)) {
            for ($i = 0; $i < count($editSaleProductList); $i++) {
              if ($editSaleProductList[$i]["product_id"] === $id) {
                ...
                $qty = intval($editSaleProductList[$i]["sale_qty"]);
                $_SESSION["editSaleProductList"][$i]["selling_price"] = $salePrice;
                $_SESSION["editSaleProductList"][$i]["sub_total"] = number_format($price * $qty, 2, '.', '');
                if (isset($editSaleProductList[$i]["edited"])) {
                  $_SESSION["editSaleProductList"][$i]["edited"] = "Y";
                }
