...
if (isset($_SESSION["user_id"]) && isset($_SESSION["username"])) {
  ...
  if ($_SERVER["REQUEST_METHOD"] === "POST") {
    $errors = validatePurchase($editPurchaseProductList);
    if (count($errors) === 0) {
      $purchase_invoice_no = mysqli_real_escape_string($conn, htmlspecialchars(stripslashes($_SESSION["edit_purchase_invoice_no"])));
      $supplier_id = $_SESSION["edit_supplier_id"];
      $supplier_name = $_SESSION["edit_supplier_name"];
      $purchase_date = $_SESSION["edit_purchase_date"];
      $current_date = date('Y-m-d H:i:s');
      $current_year = date('Y');
      try {
        foreach ($editPurchaseProductList as $product) {
          ...
          if (isset($product["edited"])) {
            if ($product["deleted"] === "Y") {
              $query = "SELECT * FROM purchases WHERE 
              purchase_invoice_no='$purchase_invoice_no' AND product_id='$product_id';";
              $result = mysqli_query($conn, $query);
              if (mysqli_num_rows($result) > 0) {
                $row = mysqli_fetch_assoc($result);
                $purchase_id = $row["purchase_id"];
                $purchase_qty = $row["purchase_qty"];
                $query1 = "DELETE FROM purchases WHERE purchase_id='$purchase_id';";
                ...
                if (mysqli_affected_rows($conn) > 0) {
                  ...
                  if (mysqli_num_rows($result) > 0) {
                    ...
                    if (mysqli_affected_rows($conn) > 0) {
                      $query3 = "DELETE FROM sku WHERE 
                      product_id='$product_id' AND purchase_invoice_no='$purchase_invoice_no';";
              ...
              $newQty = intval($purchase_qty);
              $diffQty = abs($newQty - $oldQty);
              $newPrice = floatval($product["purchase_price"]);

              if ($newQty < $oldQty) {
                $query = "SELECT * FROM sku WHERE product_id='$product_id' AND purchase_invoice_no='$purchase_invoice_no' 
                          AND status='purchased' ORDER BY sku_id DESC;";
                $result = mysqli_query($conn, $query);
                $rows = mysqli_fetch_all($result, MYSQLI_ASSOC);
                for ($i = 0; $i < $diffQty; $i++) {
                  $sku_id = $rows[$i]["sku_id"];
                  $query1 = "DELETE FROM sku WHERE sku_id='$sku_id';";
                  $result1 = mysqli_query($conn, $query1);
                }
                if (mysqli_affected_rows($conn) > 0) {
                  $query = "SELECT * FROM inventory WHERE product_id='$product_id';";
                  ...
                  if (mysqli_num_rows($result) > 0) {
                    ...
                    if (mysqli_affected_rows($conn) > 0) {
                      $query3 = "UPDATE purchases SET purchase_qty='$purchase_qty', purchase_price='$purchase_price', sub_total='$sub_total' 
                                WHERE purchase_invoice_no='$purchase_invoice_no' AND product_id='$product_id';";
                      $result3 = mysqli_query($conn, $query3);
                    }
                  }
                }
        ...
        if (mysqli_affected_rows($conn) > 0) {
          $query4 = "UPDATE purchases SET 
          supplier_id='$supplier_id', supplier_name='$supplier_name', purchase_date='$purchase_date', grand_total='$grand_total', 
          date_updated='$current_date' WHERE purchase_invoice_no='$purchase_invoice_no';";
          $result4 = mysqli_query($conn, $query4);
          $query5 = "UPDATE sku SET purchase_date='$purchase_date', date_updated='$current_date' 
          WHERE purchase_invoice_no='$purchase_invoice_no' AND status='purchased';";
          $result5 = mysqli_query($conn, $query5);
          unset($_SESSION["edit_purchase_date"]);
  ...
  <body>
    ...
    <section class="content">
      <div class="purchase-form-container">
            ...
            <div class="row">
              <div class="col-lg-4 col-md-4 col-sm-12 mb-3">
                ...
                <input type="text" name="supplier_name" class="form-control" id="supplier_name"
                  value="<?= $_SESSION["edit_supplier_name"] ?? "" ?>" required disabled />
              </div>
              <div class="col-lg-4 col-md-4 col-sm-12 mb-3">
                ...
                <input type="text" name="purchase_invoice_no" class="form-control" id="purchase_invoice_no"
                  value="<?= $_SESSION["edit_purchase_invoice_no"] ?? "" ?>" required disabled />
              </div>
              <div class="col-lg-4 col-md-4 col-sm-12 mb-3">
                <label for="purchase_date" class="form-label">Purchase Date</label>
                <input type="date" name="purchase_date" class="form-control" id="purchase_date"
                  value="<?= $_SESSION["edit_purchase_date"] ?? "" ?>" required disabled />
              </div>
            </div>
          </form>
        </div>
        ...
        <form method="post" action="confirm_edit_purchase" enctype="multipart/form-data" novalidate>
          <div class="row">
            <div class="mb-3">
              <button type="submit" class="btn btn-success btn-update">Confirm</button>
              <a class="btn btn-danger btn-cancel"
                href="edit_purchase?purchaseId=<?= $_SESSION["edit_purchase_invoice_no"] ?>">Back</a>
            </div>
          </div>
        </form>
