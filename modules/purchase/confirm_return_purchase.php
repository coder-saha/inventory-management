...
if (isset($_SESSION["user_id"]) && isset($_SESSION["username"])) {
  ...
  if ($_SERVER["REQUEST_METHOD"] === "POST" && count($returnPurchaseProductList) > 0) {
    try {
      foreach ($returnPurchaseProductList as $product) {
        $product_id = $product["product_id"];
        $sku = $product["sku"];
        $purchase_invoice_no = $product["purchase_invoice_no"];
        // update sku to purchased and remove purchase invoice no
        $query1 = "UPDATE sku SET return_date='$return_date', status='returned', date_updated='$current_date' WHERE sku='$sku';";
        ...
        if (mysqli_affected_rows($conn) > 0) {
          ...
          if (mysqli_affected_rows($conn) > 0) {
            // update purchase invoice quantity, prices and date updated
            $query = "SELECT * FROM purchases WHERE purchase_invoice_no='$purchase_invoice_no' AND product_id='$product_id';";
            $result = mysqli_query($conn, $query);
            $row = mysqli_fetch_assoc($result);
            $purchase_qty = intval($row["purchase_qty"]) - 1;
            $purchase_price = floatval($row["purchase_price"]);
            $sub_total = floatval($row["sub_total"]) - $purchase_price;
            $grand_total = floatval($row["grand_total"]) - $purchase_price;
            $query3 = "UPDATE purchases SET purchase_qty='$purchase_qty', sub_total='$sub_total' WHERE purchase_invoice_no='$purchase_invoice_no' AND product_id='$product_id';";
            if ($purchase_qty === 0) {
              $query3 = "DELETE FROM purchases WHERE purchase_invoice_no='$purchase_invoice_no' AND product_id='$product_id';";
            }
            $result3 = mysqli_query($conn, $query3);
            $query4 = "UPDATE purchases SET grand_total='$grand_total', date_updated='$current_date' WHERE purchase_invoice_no='$purchase_invoice_no';";
  ...
  <body>
    ...
    <section class="content">
      <div class="purchase-form-container">
        ...
        <form method="post" action="confirm_return_purchase" enctype="multipart/form-data" novalidate>

          <div class="row">
            <div class="mb-3">
              <button type="submit"
                class="btn btn-success btn-update <?= count($returnPurchaseProductList) > 0 ? "" : "disabled" ?>">Proceed</button>
              <a class="btn btn-danger btn-cancel" href="return_purchase">Back</a>
            </div>
          </div>
        </form>
