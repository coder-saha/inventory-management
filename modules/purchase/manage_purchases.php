...
if (isset($_SESSION["user_id"]) && isset($_SESSION["username"])) {
  ...
  try {
    $result = mysqli_query($conn, "SELECT count(*) FROM purchases WHERE deleted='0' GROUP BY purchase_invoice_no;");
  } catch (Exception $e) {
    writeLog($e->getMessage());
  }
  $record_count = floatval(mysqli_fetch_assoc($result)["count(*)"]);
  ...
  if (($page * $records) > ($record_count + $records)) {
    header("Location: manage_purchases?page=" . $page_count . "&records=" . $records);
  } else {
    try {
      $query = "SELECT purchase_invoice_no, supplier_name, purchase_date, grand_total, date_added, date_updated FROM purchases WHERE deleted='0' GROUP BY purchase_invoice_no LIMIT $records OFFSET $start_from;";
  ...
  <body>
    ...
    <section class="content">
      ...
      <div>
        <div class="button-div">
          <a class="btn btn-primary" href="add_purchase">Add New</a>
          <button class="btn btn-danger btn-delete hidden" onclick="deletePurchases()">Delete Selected</button>
        </div>
        <div class="search-div">
          <div class="input-group">
            <input type="text" class="form-control" name="purchaseId" id="purchaseId" value="" placeholder="Search Purchases" />
            <span class="input-group-text btn-search" name="search" title="Search Purchase" onclick="searchPurchase()">
              <i class="bi bi-search"></i>
            </span>
          </div>
        </div>
        <a class="btn btn-primary btn-dashboard hidden"
          href="manage_purchases?page=<?php echo $page . "&records=" . $records; ?>">Back to Purchases</a>
                ...
                while ($row = $result->fetch_assoc()) {
                  ...
                  <tr>
                    ...
                    <td>
                      ...
                      <a href="../utils/print_barcode?mode=purchaseId&purchaseId=<?php echo $row['purchase_invoice_no'] ?>" target="_blank" class="action-btn">
    ...
    <script>
      ...
      purchaseId.addEventListener("keyup", function (event) {
        if (event.keyCode == 13) {
          searchPurchase();
        }
      });
      ...
      function deletePurchases() {
        if (confirm("Are you sure you want to delete these purchases ?")) {
          $.post("delete_purchases",
            { "purchaseIds": JSON.stringify(checkList) },
            function (data) {
              if (data === "success") {
                window.location.replace("manage_purchases?page=" + "<?php echo $page ?>" + "&records=" + "<?php echo $records ?>");
              } else {
                $(".msg-box").html(data);
              }
            }
      ...
      function applyPagination(element) {
        ...
        window.location.replace("manage_purchases?page=" + pageElements[3].value + "&records=" + pageElements[8].value);
      }

      function firstPage() {
        const recordSize = "<?php echo $records ?>";
        const page = 1;
        window.location.replace("manage_purchases?page=" + page + "&records=" + recordSize);
      }
