...
if (isset($_SESSION["user_id"]) && isset($_SESSION["username"])) {
  ...
  try {
    $result = mysqli_query($conn, "SELECT count(*) FROM products WHERE deleted='0';");
  } catch (Exception $e) {
  ...
  if (($page * $records) > ($record_count + $records)) {
    header("Location: manage_products?page=" . $page_count . "&records=" . $records);
  } else {
    try {
      $query = "SELECT * FROM products WHERE deleted='0' ORDER BY product_id LIMIT $records OFFSET $start_from;";
  ...
  <body>
    ...
    <section class="content">
      ...
      <div>
        <div class="button-div">
          ...
          <button class="btn btn-danger btn-delete hidden" onclick="deleteProducts()">Delete Selected</button>
        </div>
        <div class="search-div">
          <div class="input-group">
            <input type="text" class="form-control" name="productId" id="productId" value="" placeholder="Search Products" />
        ...
        <a class="btn btn-primary btn-dashboard hidden"
          href="manage_products?page=<?php echo $page . "&records=" . $records; ?>">Back to Products</a>
    ...
    <script>
      ...
      function deleteProducts() {
        if (confirm("Are you sure you want to delete these products ?")) {
          $.post("delete_products",
            { "productIds": JSON.stringify(checkList) },
            function (data) {
              if (data === "success") {
                window.location.replace("manage_products?page=" + "<?php echo $page ?>" + "&records=" + "<?php echo $records ?>");
              } else {
                $(".msg-box").html(data);
              }
            }
      ...
      function applyPagination(element) {
        ...
        window.location.replace("manage_products?page=" + pageElements[3].value + "&records=" + pageElements[8].value);
      }

      function firstPage() {
        const recordSize = "<?php echo $records ?>";
        const page = 1;
        window.location.replace("manage_products?page=" + page + "&records=" + recordSize);
      }
