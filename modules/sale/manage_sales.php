...
if (isset($_SESSION["user_id"]) && isset($_SESSION["username"])) {
  ...
  try {
    $result = mysqli_query($conn, "SELECT count(*) FROM sales WHERE deleted='0' GROUP BY sale_invoice_no;");
  } catch (Exception $e) {
  ...
  if (($page * $records) > ($record_count + $records)) {
    header("Location: manage_sales?page=" . $page_count . "&records=" . $records);
  } else {
    try {
      $query = "SELECT sale_invoice_no, customer_name, sale_date, grand_total, date_added, date_updated FROM sales WHERE deleted='0' GROUP BY sale_invoice_no LIMIT $records OFFSET $start_from;";
  ...
  <body>
    ...
    <section class="content">
      ...
      <div>
        <div class="button-div">
          <a class="btn btn-primary" href="add_sale">Add New</a>
          <button class="btn btn-danger btn-delete hidden" onclick="deleteSales()">Delete Selected</button>
        </div>
        <div class="search-div">
          <div class="input-group">
            <input type="text" class="form-control" name="saleId" id="saleId" value="" placeholder="Search Sales" />
            <span class="input-group-text btn-search" name="search" title="Search Sale" onclick="searchSale()">
              <i class="bi bi-search"></i>
            </span>
          </div>
        </div>
        <a class="btn btn-primary btn-dashboard hidden"
          href="manage_sales?page=<?php echo $page . "&records=" . $records; ?>">Back to Sales</a>
    ...
    <script>
      ...
      saleId.addEventListener("keyup", function (event) {
        if (event.keyCode == 13) {
          searchSale();
        }
      });
      ...
      function deleteSales() {
        if (confirm("Are you sure you want to delete these sales ?")) {
          $.post("delete_sales",
            { "saleIds": JSON.stringify(checkList) },
            function (data) {
              if (data === "success") {
                window.location.replace("manage_sales?page=" + "<?php echo $page ?>" + "&records=" + "<?php echo $records ?>");
              } else {
                $(".msg-box").html(data);
              }
            }
      ...
      function applyPagination(element) {
        ...
        window.location.replace("manage_sales?page=" + pageElements[3].value + "&records=" + pageElements[8].value);
      }

      function firstPage() {
        const recordSize = "<?php echo $records ?>";
        const page = 1;
        window.location.replace("manage_sales?page=" + page + "&records=" + recordSize);
      }
